...
spd_plot <- ggplot(data = by_county_changes, aes(y = PrDens, x = calBP)) +
  geom_line() +
  geom_point(aes(y = PrDens, x = MedianBP), size = 2.75, color = "red", inherit.aes = FALSE) +
  geom_label_repel(aes(y = PrDens, x = MedianBP, label = SiteNameID), color = "red", cex = 5,
    show.legend  = F,
  ...
  scale_y_continuous(breaks = function(x) seq(0, max(x), length.out = 3),
                     labels = function(y) sprintf("%.2f", y)) +
  facet_wrap(~factor(county_st, levels=unique(by_county_changes$county_st)), scales = "free_y", ncol = 1) +
  xlab("Years cal BP") +
