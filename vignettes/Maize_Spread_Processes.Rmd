...
DK.caldates = calibrate(x = pima$Age,
                        errors = pima$Error,
                        calCurves = 'intcal20')

DK.bins = binPrep(sites = pima$SiteIDName,
                  ages = pima$Age,
                  h = 100)

DK.randates = sampleDates(DK.caldates,
                          #bins = DK.bins,
                          nsim = 100,
                          verbose = FALSE)

D.ckde = ckde(DK.randates, timeRange = c(6000, 1500), bw = 200)
...
expnull <-
  modelTest(
    ...
    #bins = DK.bins,
    nsim = nsim,
    timeRange = c(6000, 1500),
    model = "exponential",
    runm = 100
  )

section_area_data <- bind_rows(perm.emedyd[["observed"]], .id = "county_st")

sw_rc_subset <- sw_rc %>% 
  dplyr::filter(county_st %in% frontier_counties)
  
cal.emedyd = calibrate(x = sw_rc_subset$Age,
                        errors = sw_rc_subset$Error, normalised = FALSE)

perm.emedyd = permTest(
  x = cal.emedyd,
  marks = sw_rc_subset$county_st,
  timeRange = c(7000, 500),
  nsim = nsim,
  runm = 50
)


perm_spd_output <- dplyr::bind_cols(dplyr::bind_rows(perm.emedyd[["observed"]], .id = "county_st"), 
...
par(mfrow = c(4, 3))
plot(perm.emedyd, focalm = 1, main = "pima, arizona")
plot(perm.emedyd, focalm = 2, main = "navajo, arizona")
plot(perm.emedyd, focalm = 3, main = "montrose, colorado")
plot(perm.emedyd, focalm = 4, main = "catron, new mexico")
plot(perm.emedyd, focalm = 5, main = "kane, utah")
plot(perm.emedyd, focalm = 6, main = "san juan, utah")
plot(perm.emedyd, focalm = 7, main = "box elder, utah")
plot(perm.emedyd, focalm = 8, main = "sevier, utah")
plot(perm.emedyd, focalm = 9, main = "salt lake, utah")
plot(perm.emedyd, focalm = 10, main = "uintah, utah")
plot(perm.emedyd, focalm = 11, main = "daggett, utah")


emedyd.spd = stackspd(
  x = cal.emedyd,
  group = sw_rc_subset$county_st,
  timeRange = c(7000, 500),
  runm = 50,
  verbos = FALSE
)
...
```{r Figure_S4, fig.cap="Figure S4. ", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.width = 15.51, fig.height = 10.32, fig.align = "left"}
...
perm_spd_output <- dplyr::bind_cols(dplyr::bind_rows(perm.emedyd[["observed"]], .id = "county_st"), 
  ...
  dplyr::mutate(pvalue = ifelse(pvalue < 0.01, paste0("p < 0.01"), paste0("p = ", sprintf("%.2f", pvalue))),
                boom_bust = dplyr::case_when(PrDens < hi & PrDens > lo ~ "medium", 
                                             PrDens > hi ~ "indianred",
                                             PrDens < lo ~ "royalblue",
                                             TRUE ~ "medium"))
...
condensed <- perm_spd_output %>% 
  ...
  dplyr::filter(dplyr::row_number() %in% c(1, n()) & boom_bust != "medium") %>% 
  dplyr::group_by(grp = str_c('Column', rep(1:2, length.out = n()))) %>%
  dplyr::mutate(rn = row_number()) %>%
  dplyr::ungroup() %>%
  dplyr::select(county_st, Date, boom_bust, grp, rn) %>% 
  tidyr::pivot_wider(names_from = grp, values_from = Date) %>% 
  dplyr::select(-rn, xmin = Column1, xmax = Column2, fill = boom_bust) %>% 
  dplyr::mutate(ymin = -Inf, ymax = Inf) %>% 
  dplyr::left_join(., ckde_test3 %>% dplyr::ungroup() %>% dplyr::select(county_st, county_st2) %>% dplyr::distinct(), by = c("county_st" = "county_st2")) %>% 
...
ggplot2::ggsave(
  filename = here::here("vignettes/figures/supplemental/Figure_S4.png"),
  plot = perm_spd_plot,
  width = 16,
  height = 18,
  dpi = 600
