...
png(
  here::here("vignettes/figures/Supp_Figure_contour.png"),
  units = "in",
  width = 15.43,
  height = 13.15,
  res = 600
...
fig_envA <- cowplot::plot_grid(NED_output, GDD_output, labels = "AUTO", label_y = 0.87, label_x = 0.09, align = "h")
ggplot2::ggsave(
  fig_envA,
  filename = here::here("vignettes/figures/fig_env.png"),
  width = 30.24,
  height = 18.26,
  dpi = 600
)
...
```{r maize_frontier, fig.cap="Figure. maize_frontier", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.width = 15.51, fig.height = 10.32, fig.align = "left"}
...
nodes1 <-
  ...
  dplyr::mutate(normal = (delta-min(delta, na.rm = T))/(max(delta, na.rm = T)-min(delta, na.rm = T))) %>%
...
names(nodes1) <- c("id", "lon", "lat", "name", "date", "delta", "normal")

nodes1 <- nodes1 %>%
  dplyr::arrange(id)

edges1 <- data.frame(from = 1:12,
                     to = 2:13,
                     weight = nodes1$normal[!is.na(nodes1$normal)],
                     color = nodes1$delta[!is.na(nodes1$delta)])

g1 <- graph_from_data_frame(edges1, directed = FALSE, vertices = nodes1)

edges_for_plot1 <- edges1 %>%
  inner_join(nodes1 %>% select(id, lon, lat), by = c('from' = 'id')) %>%
  rename(x = lon, y = lat) %>%
  inner_join(nodes1 %>% select(id, lon, lat), by = c('to' = 'id')) %>%
  rename(xend = lon, yend = lat)
assert_that(nrow(edges_for_plot1) == nrow(edges1))

country_shapes1 <- geom_polygon(aes(x = long, y = lat, group = group),
                                data = map_data('state'),
                                fill = "white", color = "black",
                                size = 0.15)
mapcoords1 <- coord_fixed(xlim = c(-114.814521, -104.062648), ylim = c(31.333254, 42.001167))
...
k <- ggplot(nodes1) +
  ...
  scale_color_continuous(name = "Degrees/Year", breaks=seq(min(edges_for_plot1$color), max(edges_for_plot1$color), length.out = 5),
                         labels = sprintf("%.5f", seq(min(edges_for_plot1$color), max(edges_for_plot1$color), length.out = 5)),
                         type = "viridis") +
  scale_size_continuous(name = "Degrees/Year",
                        breaks=seq(min(edges_for_plot1$color), max(edges_for_plot1$color), length.out = 5),
                        labels = sprintf("%.5f", seq(min(edges_for_plot1$color), max(edges_for_plot1$color), length.out = 5))) +    # scale for node size
  geom_point(
    aes(x = lon, y = lat),
    # draw nodes
    shape = 21,
  ...
  theme(legend.position = c(0.805, 0.76),
        ...
        legend.title = element_text(size = 21, face = "bold", family = "Helvetica"))
