...
# See GitHub repo for more information: https://github.com/Archaeo-Programmer/cropSpreadR

# devtools::install()
# devtools::load_all()
library(cropSpreadR)
library(cropDiffusionR)
# Loading additional libraries as having an issue with knitting this rmd. 
library(raster)
library(tidyverse)
...
# Load temperature anomaly reconstruction from paleoMAT package.
temp_anom <-
  readr::read_csv(
...
# Load location data for each sample. Site locations are not saved in this repository, as we want to protect archaeological site locations.
sf::sf_use_s2(FALSE)
...
MDB <- cropDiffusionR::maizeDB %>%
  dplyr::left_join(.,
                   readr::read_csv(
                     stringr::str_remove(
                       here::here("maize_database/Maize_Database_Locations_DONT_DISTRIBUTE.csv"),
                       "cropSpreadR/"
                     ),
...
# If you do not have your own location information, then you can run the code below using mostly coarse scale location data provided by the p3k14c package (which primarily will have county centroids). If needed, you can uncomment this code (lines 77-102) and run instead of lines 52-74 above. However, some of the samples may not be available in the p3k14c package, and locations could vary up to at least 10 km from the actual site. p3k14c_data has a column that details the location accuracy. Please refer to Table 2 in https://doi.org/10.1038/s41597-022-01118-7 for more information on the p3k14c location accuracy codes.
# sf::sf_use_s2(FALSE)
# 
# MDB <- cropDiffusionR::maizeDB %>%
#   dplyr::left_join(.,
...
#   dplyr::mutate(
#     Long = dplyr::case_when(SiteName == "Coxcatlan" ~ -97,
#                             TRUE ~ Long),
...
# Get modern annual accumulated growing degree days (GDD) (°C) at each sample location. 
...
# For USA, we use PRISM (~800 meters) for modern annual accumulated GDD (°C) data.
MDB_gdd_prism <- MDB %>%
  sf::st_as_sf(coords = c("Long", "Lat"), crs = 4326) %>%
  dplyr::select(LabID) %>%
  cropDiffusionR::extract_GDD(gdd_raster = cropSpreadR::PRISM_annual_gdd,  sites = .)

# Combine GDD together (i.e., coalesce), where PRISM takes precedence over WorldClim data.
MDB_gdd <-
...
# Get all maize samples from each site that are north of 30° latitude and less than AD 1500.
full_distribution <- MDB_env %>%
  dplyr::filter(Lat > 30 & Date < 1500) %>%
  dplyr::mutate(sample = "Full Maize Database") %>%
  dplyr::filter(!LabID %in% maize_earliest_distribution$LabID)

# Combine data together, so that we can plot both sets as histograms on the same plot.
...
swus_date_distribution_stacked
...
```{r Figure_2, fig.cap="Figure 2. Spread rates for maize beginning from Guilá Naquitz to sites in the SWUS on the MNF as defined in the text. A) Only change in northing contributes to the rates measured. B) As in A, except rates are expressed as Euclidean distances.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}

# Get the maize northern frontier starting in Mexico, then calculate euclidean distance between sites and latitude North distance.
nodes <-
  maize_frontier_mexico %>%
  dplyr::mutate(
  ...
  # Calculate the distance between rows and store distance on the arrival location row.
  dplyr::mutate(
    # First, get the next row. Lag or row_number() - 1 does not work for geometry; thus, we had to use this to get the lead row.
    lead_row = geometry[dplyr::row_number() + 1],
    # Second, calculate the distance between the rows using euclidean distance. Then, we use lag so that it returns it for the later sample (arrival location).
    # We use lag since we cannot use it on the geometry and had to use lead instead.
    dist_deg = as.numeric(dplyr::lag(
      sf::st_distance(geometry, lead_row, by_element = TRUE, which = "Euclidean")
    )),
    # Convert degrees to km. We use the approximate 110.574 for the calculation (http://www.thegreenwichmeridian.org/tgm/articles.php?article=5).
    dist_km = dist_deg * 110.574,
    # Determine the change in degrees distance from the euclidean distance calculation above.
    delta_deg = dist_deg / (Date - dplyr::lag(Date)),
    # Determine the change in km distance from the euclidean distance calculation above.
    delta_km = dist_km / (Date - dplyr::lag(Date)),
    # Get the change in degrees latitude between sites, and limit the number of decimal points.
    delta_lat_deg = ((Lat - dplyr::lag(Lat))) / (Date - dplyr::lag(Date)),
    delta_lat_deg = as.numeric(sprintf("%.4f", delta_lat_deg)),
    # Get the change in km latitude between sites.
...
# Keep only select columns that we will use for plotting.
nodes1 <- nodes %>%
  dplyr::select(
    c(
      "id",
...
# Get shapefiles for world boundaries.
country_shapes1 <-
...
# Set lat/long limits for plotting.
mapcoords1 <-
...
# Join the edges together with the node data.
...
# Check that the edge list and the joins with node data still produce the same number of rows.
...
# Select some specific columns, which we will use to create Table 1 below (in the next code chunk).
...
# Pull out just LabID and the two sigma columns, so that we can extract the two-sigma range for each LabID, which we will include in Table 1.
sigma_range <-
  maize_spread_table %>%
  dplyr::select(LabID, dplyr::starts_with("TwoSigma")) %>%
  tidyr::pivot_longer(-LabID) %>%
  dplyr::filter(!is.na(value)) %>%
  dplyr::group_by(LabID) %>%
  dplyr::filter(dplyr::row_number() %in% c(1, dplyr::n())) %>%
  tidyr::separate(value, into = c("early", "late"), sep = " to ") %>%
  tidyr::pivot_longer(c(early, late), names_to = "date") %>%
  dplyr::mutate(range = paste0(max(value), " to ", min(value))) %>%
  dplyr::select(-c(name, date, value)) %>%
  dplyr::ungroup() %>%
  dplyr::distinct()

# Select particular columns and add two sigma data to table.
table1 <- maize_spread_table %>% 
  dplyr::select(LabID:elevation) %>%
  dplyr::left_join(., sigma_range, by = "LabID") %>% 
  dplyr::select(LabID:Country, Age, Error, range, MedianBP, Date, everything()) %>% 
  # We use paperpile to compile references in a google doc, so we just put a place holder for the column here.
  dplyr::mutate(References = "")
...
# readr::write_excel_csv(table1,
#                        here::here("vignettes/tables/Table_1.csv"))

knitr::kable(table1,
             caption = "Table 1. Sites on the Maize Northern Frontier (MNF), beginning with Guilá Naquitz. Here, we include information on the uncalibrated and calibrated dates, as well as other information on elevation and GDD.")

```

```{r Table2, echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'asis'}

# Table 2 just focuses on the distance and change in distance data. 
table2 <- maize_spread_table %>% 
  dplyr::select(LabID:SiteName, Date, dist_deg:delta_lat_km) %>% 
  dplyr::mutate(dplyr::across(dist_deg:delta_lat_km, ~ sprintf("%.3f", .x)))
...
# readr::write_excel_csv(table2,
#                        here::here("vignettes/tables/Table_2.csv"))

knitr::kable(table2,
             caption = "Table 2. Distances and spread rates for sites on the Maize Northern Frontier (MNF), beginning with Guilá Naquitz. Distances and maize spread rates are between a previous row and a current row. For example, Guilá Naquitz is blank because the distance between Guilá Naquitz and Las Capas is provided in the Las Capas row.")
             
```

```{r Figure_3, fig.cap="Figure 3. Spread rates for maize beginning from Las Capas to sites in the SWUS on the MNF as defined in the text. A) Only change in northing contributes to the rates measured. B) As in A, except rates are expressed as Euclidean distances.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}

# Use the same data as above for Figure 2; however, we remove Mexico sites from the frontier to focus just on the southwestern United States.
nodes2 <- nodes1 %>%
  dplyr::filter(name != "Guilá Naquitz") %>%
  dplyr::mutate(id = dplyr::row_number())

# Get state shapefiles for plotting.
country_shapes1 <-
  ggplot2::geom_polygon(
    aes(x = long, y = lat, group = group),
    data = map_data('state'),
...
# Use cowplot to put the 2 figures together.
Figure_3 <-
  cowplot::plot_grid(
...
```{r Figure_4, fig.cap="Figure 4. Location of sites on the maize northern frontier and other sites mentioned in the text relative to A) elevation and B) annual accumulated GDDs (°C).", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}
...
# We add Turkey Pen as an additional site to plot, but we distinguish between the Maize Northern Frontier and Other sites that we want to plot. 
...
# Use cowplot to put the GDD and elevation maps in the same figure.
fig_envA <-
  cowplot::plot_grid(
    NED_output,
    GDD_output,
...
```{r Figure_5, fig.cap="Figure 5. Box plots showing elevation, GDD, and latitude through time for the earliest maize sample from each of 275 sites in our database (A. Gillreath-Brown and Kohler n.d. 2022), excluding 19 sites post-dating AD 1500 or south of 30° Latitude. Samples are binned by 500-year periods and plotted on the midpoint of each bin.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}

# Limit box plot to just be greater than 30° N and less than AD 1500. We want only the oldest samples from each site, then we group in 500 year time bins for plotting the box plots.  
MDB_env2 <- MDB_env %>%
  dplyr::filter(Lat >= 30 & Date < 1500) %>%
  dplyr::group_by(SiteIDName) %>%
  dplyr::slice_min(Date, with_ties = FALSE) %>%
...
# Get the average for the 3 variables. 
...
```{r multiple_linear_regression, echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'asis'}

# Limit regressions to sites greater than 30° N and less than AD 1500, and only to the Maize Northern Frontier sites.
mlr <- MDB_env %>%
  dplyr::group_by(SiteIDName) %>%
  dplyr::slice_min(Date) %>%
  dplyr::filter(Lat >= 30 & Date < 1500) %>%
...
# Rescale the 4 variables.
mlr2 <- mlr %>%
  dplyr::select(Date, Lat, elevation, GDD) %>% 
  dplyr::mutate(dplyr::across(everything(), scales::rescale))

# Calculate the simple linear regressions for each combination of variables for plotting on the dag.
...
# Extract all the statistics for the linear regressions.
stats_table <-
...
# Create connections in the dag.
dag_out <- ggdag::dagify(GDD ~ Latitude + Elevation,
                         Date ~ GDD,
                         Date ~ Latitude + Elevation,
                         coords = dag_coords) %>%
  # Put in a tidy format.
  ggdag::tidy_dagitty() %>%
  # Use a custom function to shorten the dag arrows on both ends.
  cropSpreadR::shorten_dag_arrows(proportion = 0.27)

# Add the slope (i.e., beta weights) to the tidy tibble, so that we can also plot the beta weights on the dag.
dag_out$data <-
  dplyr::left_join(
    x = dag_out$data,
    stats_table %>%
...
```{r Figure_7, fig.cap="Figure 7. Regression of the rate of maize dispersal per year along the MNF on the estimated summer temperature anomaly (°C) (Andrew Gillreath-Brown, Bocinsky, and Kohler n.d. 2022) (r2 = 0.16; p = 0.20). Summer temperature estimates increase from left to right on the x-axis. The gray band represents a 95% confidence interval.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}
...
```{r Figure_8, fig.cap="Figure 8. The 12 counties showing A) number of radiocarbon samples and B) number of sites with radiocarbon samples in the four-state SWUS with a site on the MNF (2 sites fall within Daggett County, Utah). The northern frontier of maize expansion was located almost entirely within the western Southwest. We still show Delta County here as it is on the MNF; however, we have excluded it from Figure 9 due to sample size.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}
...
```{r Figure_9, fig.cap="Figure 9. The MNF relative to the Monte-Carlo simulation test for SPDs for the counties in the four-state southwestern United States that are on the MNF at some point in time (see Gillreath-Brown and Kohler n.d., Figure 6 and site locations in Figure 2). Red points locate the earliest known maize in each county. Sample sizes for the SPDs shown in parentheses and the p value for the global significance test for the summed probabilities. The grey band (or envelope) represents the 95% confidence interval, which is where we would expect 95% of the radiocarbon data to be. Note variability in scales on y axis.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}
...
condensed <- monte_carlo_sim %>%
  ...
  dplyr::select(-time) %>% 
  dplyr::group_by(grp = stringr::str_c('Column', rep(1:2, length.out = dplyr::n()))) %>%
...
```{r estimating_maize_departure, echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'asis'}

maize_spread_rates <- readr::read_csv(here::here("vignettes/tables/Table_2.csv"), show_col_types = FALSE) %>% 
  dplyr::select(`Lab ID`, `Site Name`, delta_kmN = `Δ km North/Year`)

max_SW_spread <- maize_spread_rates %>% 
  dplyr::filter(!`Site Name` %in% c("Guilá Naquitz", "Las Capas")) %>% 
...
out <- data.frame(type = c("Max SWUS from MNF", "Las Capas to Six Toe Shelter", "Las Capas to Willard Mounds"),
                  rate = c(max_spread, LC_ST_rate, LC_WM_rate)) %>% 
  dplyr::mutate(time = dist / rate)

names(out) <- c("Spread Rate", "Rate (km N/Year)", "Time Elapsed (Years)")

knitr::kable(out,
             caption = "Table S2. Summary for the spread rates of maize. Here, we show the maximum rate between any two sites on the MNF, then we show the rate between Las Capas and two other sites, Six Toe Shelter and Willard Mounds. Time elapsed is the amount of time that it would take to reach the Balsas River Basin at the given spread rate. We use the distance between Las Capas to the centroid of the Balses River Basin to calculate the time elapsed.")

```

# Supplemental Figures

```{r Figure_S1, fig.cap="Figure S1. Spread rates for maize beginning from Las Capas to sites in the SWUS on the MNF as defined in the text. However, here we show a route splitting at Three Fir Shelter into an eastern and western trajectories along which new rates are calculated. A) Only change in northing contributes to the rates measured. B) As in A, except rates are expressed as Euclidean distances.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}
...
nodesS1 <-
  ...
  dplyr::select(grp, Long, Lat, SiteName, Date, LabID) %>%
  dplyr::group_by(grp) %>%
  dplyr::rename(geoms = geometry) %>% 
  dplyr::mutate(
    lead_row = geoms[dplyr::row_number() + 1],
    dist_deg = as.numeric(dplyr::lag(
      sf::st_distance(geoms, lead_row, by_element = TRUE, which = "Euclidean")
    )),
    dist_km = dist_deg * 110.574,
    delta_deg = dist_deg / (Date - dplyr::lag(Date)),
    delta_km = dist_km / (Date - dplyr::lag(Date)),
...
edges1 <- data.frame(
  from = c(1:2, 4:7, 9:14),
  to = c(2:3, 5:8, 10:15),
  color = nodesS1$delta_lat_km[!is.na(nodesS1$delta_lat_km)]
)
...
edges_for_plot1 <- edges1 %>%
  dplyr::inner_join(nodesS1 %>% dplyr::select(id, lon, lat), by = c('from' = 'id')) %>%
  dplyr::rename(x = lon, y = lat) %>%
  dplyr::inner_join(nodesS1 %>% dplyr::select(id, lon, lat), by = c('to' = 'id')) %>%
  dplyr::rename(xend = lon, yend = lat)
...
edges1 <- data.frame(
  from = c(1:2, 4:7, 9:14),
  to = c(2:3, 5:8, 10:15),
  color = nodesS1$delta_km[!is.na(nodesS1$delta_km)]
)
...
```{r TableS1, echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'asis'}

maize_spread_table <- nodesS1 %>%
  dplyr::select(-c(id, Long, Lat)) %>%
  dplyr::filter(!(SiteName == "Three Fir Shelter" & is.na(dist_deg))) %>% 
  dplyr::mutate(dplyr::across(dist_deg:delta_lat_km, ~ sprintf("%.3f", .x))) %>% 
  dplyr::relocate(LabID, .before = "SiteName") %>% 
  dplyr::arrange(grp, Date)
...
# readr::write_excel_csv(maize_spread_table, here::here("vignettes/tables/supplemental/Table_S1.csv"))

knitr::kable(maize_spread_table,
             caption = "Table S1. Distances and spread rates for sites on the Maize Northern Frontier (MNF), beginning with Guilá Naquitz. However, the rates are for a split route with new rates calculated, where it splits at Three Fir Shelter into an eastern and western route. Distances and maize spread rates are between a previous row and a current row. For example, Guilá Naquitz is blank because the distance between Guilá Naquitz and Las Capas is provided in the Las Capas row. See also Figure S1.")

```

```{r Figure_S2, fig.cap="Figure S2. Distribution of the number of samples at each site in each county on the MNF.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}
...
```{r Figure_S5, fig.cap="Figure S5. The MNF relative to the random mark permutation test for SPDs for the counties in the four-state southwestern United States that are on the MNF through time (see Gillreath-Brown and Kohler n.d., Figure 6 and site locations in Figure 2). Red points locate the earliest known maize in each county. Sample sizes for the SPDs shown in parentheses and the global p value, which means that most of the data falls outside of the envelope. Note variability in scales on y axis.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}
...
```{r Figure_S6, fig.cap="Figure S6. Box plots showing elevation, GDD, and latitude through time for the earliest maize sample from each of 275 sites in our database (Gillreath-Brown and Kohler n.d. 2022), excluding 19 sites post-dating AD 1500 or south of 30° Latitude. Samples are binned by 100-year periods and plotted on the midpoint of each bin. This is the same as Figure 5 except here we bin the data into 100 year time bins.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.align = "left"}

MDB_env2_Figure_S6 <- MDB_env %>%
  dplyr::filter(Lat >= 30 & Date < 1500) %>%
  dplyr::group_by(SiteIDName) %>%
  dplyr::slice_min(Date, with_ties = FALSE) %>%
  dplyr::group_by(gr = cut(
...
env_summaries_Figure_S6 <- MDB_env2_Figure_S6 %>%
  dplyr::summarise(dplyr::across(c(Latitude, GDD, Elevation), ~ mean(.x, na.rm = TRUE)))
...
# ggplot2::ggsave(
#   filename = here::here("vignettes/figures/supplemental/Figure_S6.png"),
#   plot = Figure_S6,
#   width = 15.51,
#   height = 10.32,
#   dpi = 600
