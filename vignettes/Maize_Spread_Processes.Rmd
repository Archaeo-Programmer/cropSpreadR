...
edges1 <- data.frame(
  from = 1:12,
  to = 2:13,
  color = nodes2$delta_lat_km[!is.na(nodes2$delta_lat_km)]
)

g1 <-
  igraph::graph_from_data_frame(edges1, directed = FALSE, vertices = nodes2)
...
edges1 <- data.frame(
  from = 1:12,
  to = 2:13,
  color = nodes2$delta_km[!is.na(nodes2$delta_km)]
)
...
env_summaries_bp <-
  ...
  dplyr::rename(`Latitude (°)` = Latitude, `Annual GDD (°C)` = GDD, `Elevation (m)` = Elevation) %>% 
...
dag_out <- ggdag::dagify(GDD ~ Latitude + Elevation,
                         ...
                         coords = dag_coords) %>%
  ggdag::tidy_dagitty() %>%
  shorten_dag_arrows(proportion = 0.27)
...
dag_figure <- dag_out %>%
  ...
  ggdag::geom_dag_point(color = "cornflowerblue",
                        alpha = 1 / 4,
                        size = 45) +
  ggdag::geom_dag_text(color = "cornflowerblue", size = 7, fontface = "bold") +
  ggdag::geom_dag_edges(aes(x = xstart, y = ystart, edge_linetype = edge_linetype), edge_color = "cornflowerblue", arrow_directed = grid::arrow(length = grid::unit(12, "pt"), type = "closed"), edge_width = 1.35) +
  geom_dag_label_repel(aes(x = x_label, y = y_label, label = slope), segment.size = 0, force = 0, size = 6, label.size = NA) +
...
anom_point <- temp_anom %>%
  ...
  dplyr::mutate(anom_25 = dplyr::lag(anom, 25),
...
lm(delta ~ anom_25, data = lags_point) %>% 
  summary()
...
lags_point$inside <-
  ifelse(lags_point$delta < dat[, "upr"] &
           lags_point$delta > dat[, "lwr"], "", as.character(lags_point$SiteName))
...
ggplot2::ggsave(
  filename = here::here("vignettes/figures/temp_anom_spreadrate.png"),
...
ckde_dates <- function(data, interval = 0.95){
  ...
  lo = apply(sw_ckde$res.matrix,
             ...
             prob = (1 - interval) / 2,
             na.rm = TRUE)
  hi = apply(sw_ckde$res.matrix,
             1,
             quantile,
             prob = interval + (1 - interval) / 2,
             na.rm = TRUE)

  data.frame(Date = 9000:0, avg, lo, hi) %>%
...
ckde_test2 <- ckde_test1 %>%
  ...
  dplyr::bind_rows(ckde_test1 %>% dplyr::filter(!SiteIDName %in% sites_to_subsample), ., ) %>%
  dplyr::select(-SiteIDName) %>%
  dplyr::group_by(county_st) %>%
  dplyr::mutate(n = dplyr::n()) %>%
  dplyr::group_by(n, .add = TRUE) %>%
  tidyr::nest() %>%
  dplyr::mutate(ckde = purrr::map(data, ckde_dates)) %>%
  dplyr::select(-data) %>%
  tidyr::unnest(ckde)
...
maize_oldest2 <-
  ...
  dplyr::select(county_st, Date, SiteNameID) %>%
  dplyr::mutate(date_maize = Date)

ckde_test3 <- ckde_test2 %>%
  dplyr::left_join(., maize_oldest2, by = c("county_st", "Date")) %>%
...
```{r Figure_S2, fig.cap="Figure S2. The MNF relative to the SPDs for the counties in the four-state southwestern United States that are on the MNF through time (see Gillreath-Brown and Kohler n.d., Figure 6 and site locations in Figure 2). Red points locate the earliest known maize in each county. Sample sizes for the SPDs shown in parentheses. Note variability in scales on y axis.", echo = FALSE, warning = FALSE, message = FALSE, error = FALSE, results = 'hide',fig.keep = 'all', fig.width = 15.51, fig.height = 10.32, fig.align = "left"}
