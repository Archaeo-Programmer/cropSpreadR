...
nodes1 <-
  ...
  dplyr::select(id, Long, Lat, SiteName, Date) %>%
  dplyr::mutate(
    lead_row = geometry[row_number() + 1],
...
maize_frontier_mexico_delta_Lat <- ggplot(nodes1) +
  ...
  theme(plot.title = element_text(size=22, face = "bold", family = "Helvetica"),
    ...
    legend.position = c(0.823, 0.8015),
    legend.key.size = unit(0.95, "cm"),
    legend.text = element_text(size = 19, family = "Helvetica"),
    legend.title = element_text(
      size = 21,
...
# Getting spread rates.
tv_bc <- MDB %>%
  dplyr::filter(SiteName %in% c("San Marcos", "Bat Cave")) %>%
